...
function FileItem({ node, depth, isLast, parentPath, onFileClick, activeFile }: FileItemProps) {
  ...
  const isActive = !isFolder && node.id === activeFile

  const handleClick = () => {
    if (isFolder) {
      setIsOpen(!isOpen)
    } else if (onFileClick && node.id) {
      onFileClick(node.id)
    }
  }
    ...
    <div className="select-none">
      ...
      >
        {depth > 0 && (
          <div className="absolute left-0 top-0 bottom-0 flex" style={{ left: `${(depth - 1) * 16 + 16}px` }}>
            <div className={cn("w-px transition-colors duration-200", isHovered ? "bg-primary/20" : "bg-border/50")} />
          </div>
        )}
        ...
        >
          {isFolder ? (
            <svg
              ...
              viewBox="0 0 6 8"
              fill="none"
              className={cn("transition-colors duration-200", isHovered ? "text-foreground/60" : "text-muted-foreground")}
            >
              <path
                d="M1 1L5 4L1 7"
                stroke="currentColor"
        ...
        {isActive && (
          <div className="absolute right-2 w-1 h-1 rounded-full bg-foreground/40" />
        )}
      </div>
      ...
      {hasChildren && (
        <div
          ...
          style={{
            maxHeight: isOpen ? `${node.children!.length * 200}px` : "0px",
          }}
        >
...
export function FileTree({ data, className, onFileClick, activeFile }: FileTreeProps) {
  return (
    <div
      className={cn(
        "bg-file-tree-bg rounded-lg border border-border/50 p-3",
        className,
      )}
    >
      <div className="flex items-center gap-2 pb-3 mb-2 border-b border-border/30">
        ...
        <span className="text-xs text-muted-foreground ml-2 tracking-widest uppercase" style={{ fontSize: "0.6rem" }}>explorer</span>
      </div>
